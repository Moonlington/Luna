...
class Funstuff:
    ...
    async def server(self, ctx):
        await self.bot.say("""__Server info for **{0.name}**__
ID: **{0.id}**
Owner: **{0.owner}**
Location: **{0.region}**
Creation: **{0.created_at}**
Users: **{1}**
Channels: **{2}**
Verification: **{0.verification_level}**
Server Icon: {0.icon_url}""".format(ctx.message.server, len(ctx.message.server.members), len(ctx.message.server.channels)))
    ...
    @commands.command(pass_context=True)
    async def quote(self, ctx, qid, *, content=None):
        if content is None:
